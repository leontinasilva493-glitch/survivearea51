  ...
  return (
    ...
    >
      ...
      <section aria-labelledby="observed-runs">
        ...
        <h2 className="section-title" id="observed-runs">One loop: 37.8K–72.8K in 2m 44s–3m 20s</h2>
        <p className="mt-4 max-w-3xl text-[var(--muted)]">Both samples are hand-calculated from visible balances. Upload timing overlaps an advertised x2 Coins event window, and the footage includes multiplayer and quest notices, so the numbers must not be sold as a normal beginner rate.</p>
        <div className="mt-8 overflow-x-auto border border-[var(--line)]">
          <table className="w-full min-w-[760px] border-collapse text-left text-sm">
            ...
            <tbody>{coinRuns.map((run) => <tr className="border-b border-[var(--line)] last:border-b-0" key={run.label}><th className="p-4 font-bold">{run.label}</th><td className="p-4">{run.start}</td><td className="p-4">{run.finish}</td><td className="p-4 font-bold text-[var(--cyan)]">{run.net}</td><td className="p-4">{run.elapsed}</td><td className="p-4 font-bold">{run.rate}</td><td className="p-4"><EvidenceLink href={run.sourceUrl}>Watch</EvidenceLink></td></tr>)}</tbody>
          </table>
        </div>
        <div className="mt-4 grid gap-3 md:grid-cols-2">{coinRuns.map((run) => <p className="terminal-panel m-0 p-4 text-xs text-[var(--muted)]" key={run.label}><strong className="text-[var(--paper)]">{run.label}:</strong> {run.note}</p>)}</div>
      </section>
      ...
      <RelatedLinks links={[["Weapons guide", "/weapons/", "See the MP7, SG, and AKM used around these runs."], ["Map Lite", "/map/", "Follow the observed route anchors."], ["Gamepass guide", "/gamepasses/", "Open the official x2 Coins product record."]]} />
    </PageShell>
  );
}
